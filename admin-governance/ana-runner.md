...
| 3 | Connector Governance | List every connector in this org with its type, visibility (public/private), and whether it is read-only or read-write. Flag anything read-write, anything public that connects to a production database, and any connection holding row-level-sensitive data that is not marked TQL-only or is still reachable by roles holding the raw-sql permission. |
