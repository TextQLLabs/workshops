...
| 5 | Import a Power BI / Tableau Dashboard | Here's a [.pbit / .twb] dashboard. Read its data model, measures, and layout — don't build yet. List every measure and map each to a governed ontology metric (flag any that don't map cleanly), propose the equivalent app layout, and tell me which tables/joins each metric needs. Then build and reconcile the default numbers exactly + one filtered slice to the dollar vs a fresh warehouse query. |
